...
def is_invite(project:Project,created_by , role:str , expires_days:int=3,invited_email:str=None)->Tuple[str , str]:
   ...
   with transaction.atomic():
    invite = Invite.objects.create(project=project , created_by=created_by , role=role , expires_at=timezone.now() + timedelta(days=expires_days))
    ...
    invite_url = f"https://yourapp.com/join?token={invite.plain_token}"
