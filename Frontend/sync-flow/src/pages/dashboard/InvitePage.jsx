...
import { Shield, User, Eye, Copy, Check, Sparkles, X, Globe, Lock, ShieldCheck, ArrowRight, CheckCircle2 } from 'lucide-react';
...
const InvitePage = () => {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("saas.io/join/invite-code-123");
    setCopied(true);
      ...
      <header className="relative z-10 w-full pt-20 -mt-10 md:-mt-22 md:pt-32 md:pb-20 px-6">
        <div className="max-w-4xl mx-auto text-center space-y-6">
          ...
          >
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Access Protocol</span>
          </motion.div>

          <motion.h1 
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className="text-5xl md:text-7xl font-black tracking-tight text-slate-600 dark:text-white leading-[0.9]"
          >
            Team Invitation
          </motion.h1>
          ...
          >
            Assign a role to generate a secure invitation link <br className="hidden md:block" /> for your project collaborators.
          </motion.p>
        </div>
      </header>
        ...
        <div className="w-full max-w-md space-y-10">
          ...
          <div className="space-y-3 md:-mt-12">
            <div className="flex items-center justify-between px-2 my-6 md:my-3 md:px-0 ">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-600 ">Select Role</span>
            </div>
            ...
            <div className="grid grid-cols-3 p-1.5 bg-slate-200/50 dark:bg-white/5 backdrop-blur-xl border border-slate-300 dark:border-white/10 rounded-2xl">
              {roles.map((role) => (
                ...
                >
                  ...
                  <span className="relative z-10 tracking-tight">{role.id}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-6">
            <button
              ...
              className="group w-full md:w-auto px-5 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold rounded-2xl shadow-2xl transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-3"
            >
              Generate Invitation
      ...
      <AnimatePresence>
        {copied && (
          ...
          >
            ...
            <span className="text-sm font-medium text-white whitespace-nowrap">Copied to clipboard</span>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
