...
const Project = () => {
  ...
  return (
    ...
    >
      ...
      {isModalOpen && viewMode === "team" && (
        <TeamProject onClose={() => setIsModalOpen(false)} />
      )}

      {/* Background blur */}
      <div className="fixed inset-0 pointer-events-none">
        <div
          className={`absolute -top-20 -right-20 w-80 h-80 rounded-full blur-[100px] opacity-20 transition-colors duration-1000 ${viewMode === "solo" ? "bg-blue-400" : "bg-purple-400"}`}
        />
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {status !== "loading" && (
            <AddCard
              ...
              label={viewMode === "solo" ? "NEW_PERSONAL" : "NEW_COLLAB"}
            />
          )}
  ...
  <div className="group cursor-pointer bg-white/90 dark:bg-white/[0.03] backdrop-blur-lg border border-black/30 dark:border-white/10 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:border-blue-500 hover:shadow-2xl hover:shadow-blue-500/10 flex flex-col h-[350px]">
    ...
    <div className="p-5 flex flex-col flex-1">
      ...
      <div className="mt-auto pt-4 border-t border-slate-50 dark:border-white/5 flex items-center justify-between">
        <span className="text-[10px] font-black text-blue-500 uppercase tracking-widest group-hover:underline cursor-pointer">
          Launch Unit
        </span>
        <ArrowRight
          size={16}
          className="text-black/70 dark:text-slate-400 group-hover:text-blue-500 transition-all group-hover:translate-x-1"
        />
      </div>
    </div>
  </div>
...
const AddCard = ({ label, onClick }) => (
  <div
    onClick={onClick}
    className="group border-2 border-dashed border-black/40 dark:border-white/10 rounded-2xl flex flex-col items-center justify-center p-6 min-h-[350px] bg-white/30 dark:bg-transparent hover:border-blue-500 hover:bg-white dark:hover:bg-blue-500/5 transition-all cursor-pointer"
  >
    ...
    <span className="font-black text-[10px] text-slate-400 tracking-[0.2em] uppercase group-hover:text-blue-600">
      {label}
    </span>
  </div>
);
