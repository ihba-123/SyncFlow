...
const Project = () => {
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 dark:bg-[#020617] text-slate-900 dark:text-white font-sans transition-colors duration-500 overflow-x-hidden">
      ...
      {/* Modal Overlay */}
      {isModalOpen && <SoloProject onClose={() => setIsModalOpen(false)} />}
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-5 py-8 md:py-16">
        <header className="flex flex-col gap-6 -mt-7 mb-10">
          ...
          <div className="w-full sm:w-[320px] p-1.5 bg-slate-200/50 dark:bg-white/5 backdrop-blur-xl border border-slate-300 dark:border-white/10 rounded-2xl flex">
            <button onClick={() => setViewMode("solo")} className={`flex-1 flex items-center justify-center space-x-2 py-2.5 rounded-xl text-xs font-black transition-all ${viewMode === "solo" ? "bg-white dark:bg-blue-600 text-blue-600 dark:text-white shadow-md ring-1 ring-slate-200 dark:ring-blue-400/50" : "text-slate-500 dark:text-slate-400"}`}>
              <User size={14} /><span>SOLO</span>
            </button>
            <button onClick={() => setViewMode("team")} className={`flex-1 flex items-center justify-center space-x-2 py-2.5 rounded-xl text-xs font-black transition-all ${viewMode === "team" ? "bg-white dark:bg-blue-600 text-blue-600 dark:text-white shadow-md ring-1 ring-slate-200 dark:ring-blue-400/50" : "text-slate-500 dark:text-slate-400"}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {isFetchingNextPage && <div className="h-40 w-full bg-blue-100/20 dark:bg-white/5 animate-pulse rounded-xl" />}
        </div>
...
const GlassCard = ({ project }) => (
  <div className="group bg-white/80 dark:bg-white/[0.03] backdrop-blur-lg border border-slate-200 dark:border-white/10 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:border-blue-500 hover:shadow-2xl hover:shadow-blue-500/10">
    <div className="relative w-full h-52 bg-slate-100 dark:bg-slate-900 border-b border-slate-100 dark:border-white/5 flex items-center justify-center">
      {project.image ? (
        <img src={project.image} alt={project.name} className="w-full h-full object-contain p-2 transition-transform duration-500 group-hover:scale-105" />
      ) : (
    ...
    <div className="p-5">
      ...
      <div className="mt-auto pt-4 border-t border-slate-50 dark:border-white/5 flex items-center justify-between">
        ...
        <ArrowRight size={16} className="text-slate-300 group-hover:text-blue-500 transition-all group-hover:translate-x-1" />
      </div>
    </div>
  </div>
);
